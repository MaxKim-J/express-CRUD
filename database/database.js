...
const connect = (app, config) => {
  ...
  database.db.on("open", () => {
    console.log(`${config.db_url}데이터베이스에 연결됨!`);
    // 스키마 생성
...
const createSchema = (app, config) => {
  ...
  db_schemas.forEach(schema => {
    ...
    console.log(`${schema.collection}모델도 정의해버렸다`);

    database[schema.schemaName] = newSchema;
    database[schema.modelName] = newModel;
    console.log(
      `${schema.schemaName}스키마랑 ${schema.modelName}모델을 디비객체에 추가`
    );
  });
  app.set("database", database);
  console.log("database객체를 app객체의 속성으로 추가해버려따!");
};

database.init = (app, config) => {
  console.log("db init 호출되버려따!");
  connect(app, config);
};
